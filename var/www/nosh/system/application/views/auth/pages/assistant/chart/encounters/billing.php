...
<div id="cpt_billing_dialog" title="CPT Helper">
...
<div id="cpt_helper_dialog" title="CPT Helper">
...
<script type="text/javascript">
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('assistant/encounters/get_insurance_id');?>",
		dataType: "json",
			...
			'Save': function() {
				...
				if (bValid) {
					...
					if(str){
						$.ajax({
							type: "POST",
							url: "<?php echo site_url('assistant/encounters/billing_save');?>/",
							data: str,
							success: function(data){
								if (data == 'Close Chart') {
	...
	$("#print_invoice").button({icons: {primary: "ui-icon-print"}}).click(function(){
		...
		if (bValid) {
			...
			window.open("<?php echo site_url('assistant/encounters/print_invoice');?>/" + ins1 + "/" + ins2 + "/" + bc);
		}
	});
	$("#print_hcfa").button({icons: {primary: "ui-icon-print"}}).click(function(){
		...
		if (bValid) {
			...
			if (ins1 == '' || ins1 == '0') {
				...
				window.open("<?php echo site_url('assistant/encounters/generate_hcfa');?>/" + ins1 + "/" + ins2 + "/" + bc);
			}
		}
	});
	$("#encounter_payment").button({icons: {primary: "ui-icon-check"}}).click(function(){
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('assistant/encounters/get_eid');?>",
			success: function(data){
	...
	jQuery("#cpt_list").jqGrid({
		url:"<?php echo site_url('assistant/encounters/procedure_codes/');?>",
		datatype: "json",
		mtype: "POST",
		...
		sortname: 'cpt_charge',
	 	...
	 	ondblClickRow: function(id){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('assistant/encounters/define_icd');?>",
				data: "icd=" + item,
				dataType: "json",
				success: function(data){
	...
	$("#add_billing_cpt").click(function(){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('assistant/encounters/get_billing_dos');?>",
			success: function(data){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('assistant/encounters/get_billing');?>",
			dataType: "json",
			success: function(data){
				if (data.message == "OK") {
