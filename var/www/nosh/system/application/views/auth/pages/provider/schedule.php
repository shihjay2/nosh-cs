...
$(document).ready(function() {
	...
	$(document).idleTimeout({
		inactivity: 3600000,
		noconfirm: 10000,
		alive_url: '<?php echo site_url("provider/schedule");?>',
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('provider/schedule/set_default_provider/');?>",
		success: function(data){
			$("#provider_schedule1").load('<?php echo site_url("provider/schedule/schedule_view");?>');
		}
	});
	$('#provider_list2').change(function() {
		...
		if(id){
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('provider/schedule/set_provider/');?>",
				data: "id=" + id,
				success: function(data){
					$("#provider_schedule1").load('<?php echo site_url("provider/schedule/schedule_view");?>');
				}
