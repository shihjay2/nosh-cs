	...
	<table>
		<tr>
			<td valign="top">
				<table>
					<tr>
						<td valign="top" id="recommendations_buttons">
							Recommendations:<br>
			...
			<td valign="top">
				<table>
					...
					<tr>
						<td><div id="button_orders_rx_status" class="orders_tooltip"><?php echo $rx_status;?></div></td>
...
<div id="edit_orders_dialog" title="Edit Orders Text">
	<form id="edit_orders_form">
		Text:<br>
		<textarea style="width:450px" rows="4" name="test" id="edit_orders_dialog_text" class="text ui-widget-content ui-corner-all"></textarea>
	</form>
</div>
...
<script type="text/javascript">
					...
					if(data.response =='true'){
						...
						$("#instructions_chosen").catcomplete({
							...
							select: function(event, ui){
								...
								$.ajax({
									type: "POST",
									url: "<?php echo site_url('provider/chartmenu/print_vivacare');?>/" + ui.item.link,
									dataType: "json",
									success: function(data){
										if (data.message == 'OK') {
											$.ajax({
	...
	$('.orders_tooltip').tooltip({
		...
		open: function(event, ui) {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('provider/encounters/tip_orders/');?>/" + id1,
				success: function(data) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('provider/encounters/tip_edit_orders/');?>/" + id1,
			dataType: 'json',
			success: function(data) {
				if (data.type != "N") {
			...
			'Save': function() {
				...
				if (bValid) {
					...
					if(str){
						...
						$.ajax({
							type: "POST",
							url: "<?php echo site_url('provider/encounters/edit_tip_orders');?>/" + b,
							data: str,
							success: function(data){
								$.jGrowl(data);
