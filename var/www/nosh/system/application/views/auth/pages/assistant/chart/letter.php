<div id="letter_dialog" title="Letter Writer">
	<form id="letter_form">
		...
		<span style="float:left;width:310px">
			Provider:<br><select name="letter_provider" id="letter_provider" style="width:290px" class="text ui-widget-content ui-corner-all"></select><br>
...
<script type="text/javascript">
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('assistant/chartmenu/get_providers/');?>",
		dataType: "json",
		success: function(data){
			if (data.message == "OK") {
				$('#letter_provider').addOption({"":"*Select a provider"});
				$("#letter_provider").addOption(data, false).removeOption("message");
			} else {
				$.jGrowl(data.message);
			}
		}
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('assistant/chartmenu/letter_template_select_list');?>",
		dataType: "json",
		success: function(data){
	...
	$('#letter_template_choose_id').change(function(){
		...
		if (a != '') {
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('assistant/chartmenu/get_letter_template');?>" + "/" + a,
				dataType: "json",
				success: function(data){
	...
	$("#letter_template_save").button({icons: {primary: "ui-icon-arrowthick-1-w"}}).click(function(){
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('assistant/chartmenu/letter_template_construct');?>",
			dataType: "json",
			success: function(data){
	...
	$("#save_letter").click(function(){
		var provider = $("#letter_provider");
		var bValid = true;
		bValid = bValid && checkEmpty(provider,"Provider");
		if (bValid) {
			var str = $("#letter_form").serialize();
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('assistant/chartmenu/print_letter');?>",
				data: str,
				dataType: 'json',
				async: false,
				success: function(data){
