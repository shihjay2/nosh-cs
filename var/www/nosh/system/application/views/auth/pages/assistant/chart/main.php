...
$(document).ready(function() {
	...
	$(document).idleTimeout({
		inactivity: 3600000,
		noconfirm: 10000,
		alive_url: '<?php echo site_url("assistant/chartmenu");?>',
	...
	$("#new_encounter_dialog").dialog({ 
		...
		open: function(event, ui) {
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('assistant/chartmenu/get_copay');?>",
				success: function(data){
		...
		buttons: {
			'Add Encounter': function() {
				...
				var provider = $("#encounter_provider");
				var bValid = true;
				...
				bValid = bValid && checkEmpty(provider,"Encounter Provider");
				if (bValid) {
					if($("#encounter_type").val() == '') {
						if(confirm('Are you sure you want to create a new encounter without an associated appointment?')){
							...
							if(str){
								$.ajax({
									type: "POST",
									url: "<?php echo site_url('assistant/encounters/new_encounter');?>",
									data: str,
									success: function(data){
										$.jGrowl(data);
	...
	$("#save_new_import").click(function() {
		...
		if (bValid) {
			...
			if(str){
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('assistant/chartmenu/documents_upload1');?>",
					data: str,
					success: function(data){
						$.jGrowl(data);
						$('#new_import_form').clearForm();
	...
	$("#cancel_new_import").click(function() {
		...
		if(id){
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('assistant/chartmenu/delete_upload');?>",
	...
	$("#encounter_provider").autocomplete({
		source: function (req, add){
			$.ajax({
				url: "<?php echo site_url('search/provider1');?>",
				dataType: "json",
				type: "POST",
				data: req,
				success: function(data){
		...
		select: function(event, ui){
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('assistant/chartmenu/get_appointments');?>/" + ui.item.id,
				dataType: "json",
				success: function(data){
		...
		change: function (event, ui) {
			if(!ui.item){
				$.jGrowl("Select provider from list only!");
				$("#encounter_provider").val("").focus();
			}
		}
	});
							...
							<tr>
								...
								<td>
									...
									<div id="import_upload_div" style="display:none"> 
										<br>
										<form id="import_upload_form" action="<?php echo site_url('assistant/chartmenu/documents_upload');?>" method="post" enctype="multipart/form-data">
							...
							<tr>
								...
								<td>
									...
									<div id="import_ccr_div" style="display:none"> 
										<br>
										<form id="import_ccr_form" action="<?php echo site_url('assistant/chartmenu/import_ccr');?>" method="post" enctype="multipart/form-data">
