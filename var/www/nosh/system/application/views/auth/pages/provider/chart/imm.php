<div id="immunizations_list_dialog" title="Immunizations">
	...
	<button type="button" id="imm_notes_button">Edit Immunization Notes</button><br>
	<hr class="ui-state-default"/>
	...
	<div id="imm_menu" style="display:none">
		<input type="button" id="add_immunization" value="Add Immunization" class="ui-button ui-state-default ui-corner-all"/>
		<input type="button" id="edit_immunization" value="Edit Immunization" class="ui-button ui-state-default ui-corner-all"/>
		<input type="button" id="delete_immunization" value="Delete Immunization" class="ui-button ui-state-default ui-corner-all"/>
		<button type="button" id="print_immunizations">Print List</button>
		<button type="button" id="csv_immunizations">Export CSV</button><br><br>
	</div>
	<div id="imm_order" style="display:none">
		<input type="button" id="add_immunization1" value="Add Immunization" class="ui-button ui-state-default ui-corner-all"/>
		<input type="button" id="edit_immunization1" value="Edit Immunization" class="ui-button ui-state-default ui-corner-all"/>
		<input type="button" id="delete_immunization1" value="Delete Immunization" class="ui-button ui-state-default ui-corner-all"/>
		<input type="button" id="vis_immunization1" value="Vacccine Information Sheets and Consent" class="ui-button ui-state-default ui-corner-all"/>
		<button type="button" id="print_immunizations1">Print List</button>
		<button type="button" id="csv_immunizations1">Export CSV</button><br><br>
	</div>
...
<div id="immunizations_vis_dialog" title="Vaccine Information Sheets">
	<ul>
		<li><a id="vis_dtap" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-dtap.pdf" target="_blank">DTaP</a></li>
        <li><a id="vis_hep_a" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-hep-a.pdf" target="_blank">Hepatitis A</a></li>
        <li><a id="vis_hep_b" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-hep-b.pdf" target="_blank">Hepatitis B</a></li>
        <li><a id="vis_hib" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-hib.pdf" target="_blank">Hib</a></li>
        <li><a id="vis_hpv" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-hpv.pdf" target="_blank">HPV</a></li>
        <li><a id="vis_flulive" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-flulive.pdf" target="_blank">Flu (Live, Intranasal)</a></li>
        <li><a id="vis_flu" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-flu.pdf" target="_blank">Flu (Inactivated)</a></li>
        <li><a id="vis_mmr" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-mmr.pdf" target="_blank">MMR</a></li>
        <li><a id="vis_mening" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-mening.pdf" target="_blank">Meningococcal</a></li>
        <li><a id="vis_pcv" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-pcv.pdf" target="_blank">Pneumococcal (PCV7)</a></li>
        <li><a id="vis_ppv" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-ppv.pdf" target="_blank">Pneumococcal (PPSV23)</a></li>
        <li><a id="vis_ipv" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-ipv.pdf" target="_blank">Polio</a></li>
        <li><a id="vis_rotavirus" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-rotavirus.pdf" target="_blank">Rotavirus</a></li>
        <li><a id="vis_shingles" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-shingles.pdf" target="_blank">Shingles</a></li>
        <li><a id="vis_td_tdap" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-td-tdap.pdf" target="_blank">Td/Tdap</a></li>
        <li><a id="vis_varicella" href="http://www.cdc.gov/vaccines/pubs/vis/downloads/vis-varicella.pdf" target="_blank">Varicella (Chickenpox)</a></li>     
	</ul>
	<hr class="ui-state-default"/>
...
<div id="imm_notes_dialog" title="Immunization Notes">
	<form id="imm_notes_form">
		<input type="hidden" id="imm_notes_old"/>
		<table>
			<tr>
				<td>Immunization Notes:</td>
				<td><textarea name="imm_notes" id="imm_notes" rows="4" style="width:500px" class="text ui-widget-content ui-corner-all"></textarea></td>
			</tr>
		</table><br>
		<button type="button" id="save_imm_notes">Save</button>
		<button type="button" id="cancel_imm_notes">Cancel</button>
	</form>
</div>
<script type="text/javascript">
	$("#immunizations_list_dialog").dialog({ 
		...
		open: function(event, ui) {
			...
			jQuery("#immunizations").jqGrid({
				url:"<?php echo site_url('provider/chartmenu/immunizations/');?>",
				datatype: "json",
				mtype: "POST",
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('provider/chartmenu/get_imm_notes/');?>",
	...
	$("#delete_immunization").click(function(){
		...
		if(item){
			if(confirm('Are you sure you want to delete this immunization?')){
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('provider/chartmenu/delete_immunization');?>",
					data: "imm_id=" + item,
					success: function(data){
						$.jGrowl(data);
	...
	$("#consent_immunization1").click(function(){
		...
		if(item) {
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('provider/chartmenu/consent_immunizations');?>",
		...
		if (success_doc == true) {
			window.open("<?php echo site_url('provider/chartmenu/print_consent');?>");
			success_doc = false;
		}
	});
	...
	$("#save_immunization").click(function(){
		...
		if (bValid) {
			...
			if(str){
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('provider/chartmenu/edit_immunization');?>",
					data: str,
					dataType: "json",
					success: function(data){
	...
	$("#save_immunization1").click(function(){
		...
		var cpt = $("#imm_cpt");
		var cvx = $("#imm_cvxcode1");
		var bValid = true;
		bValid = bValid && checkEmpty(imm,"Immunization");
		bValid = bValid && checkEmpty(cpt,"CPT");
		bValid = bValid && checkEmpty(cvx,"CVX");
		if (bValid) {
			var str = $("#edit_immunization_form1").serialize();		
			if(str){
				$.ajax({
					type: "POST",
					url: "<?php echo site_url('provider/chartmenu/edit_immunization1');?>",
					data: str,
					dataType: "json",
					success: function(data){
						if (data.message == 'Close Chart') {
	...
	$('#save_orders_imm').click(function(){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('provider/encounters/orders_imm_save');?>",
	...
	$('#consent_immunization1').button();
	$("#print_immunizations").button({
		icons: {
			primary: "ui-icon-print"
		}
	});
	$("#print_immunizations").click(function() {
		window.open("<?php echo site_url('provider/chartmenu/print_immunization_list');?>/");
	});
	$("#print_immunizations1").button({
		icons: {
			primary: "ui-icon-print"
		}
	});
	...
	$("#csv_immunizations").button();
	$("#csv_immunizations").click(function() {
		window.location = "<?php echo site_url('provider/chartmenu/csv_immunization');?>/";
	});
	$("#csv_immunizations1").button();
	...
	$("#imm_notes_button").click(function(){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('provider/chartmenu/get_imm_notes1/');?>",
			success: function(data){
				$('#imm_notes').val(data);
				$('#imm_notes_old').val(data)
			}
		});
		$('#imm_notes_dialog').dialog('open');
	});
	$("#save_imm_notes").button({
		icons: {
			primary: "ui-icon-disk"
		}
	});
	$("#save_imm_notes").click(function(){
		var str = $("#imm_notes_form").serialize();		
		if(str){
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('provider/chartmenu/edit_imm_notes');?>",
				data: str,
				success: function(data){
					if (data == 'Close Chart') {
						window.location = "<?php echo site_url();?>";
	...
	$("#cancel_imm_notes").button({
		icons: {
			primary: "ui-icon-close"
		}
	});
	$("#cancel_imm_notes").click(function(){
		$('#imm_notes_form').clearForm();
		$('#imm_notes_dialog').dialog('close');
	});
</script>
