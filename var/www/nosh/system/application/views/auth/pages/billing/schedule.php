...
$(document).ready(function() {
	...
	$(document).idleTimeout({
		inactivity: 3600000,
		noconfirm: 10000,
		alive_url: '<?php echo site_url("billing/schedule");?>',
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('billing/schedule/get_last_provider/');?>",
		dataType: "json",
		success: function(data){
			if (data.message == "Y") {
				$("#provider_schedule1").load('<?php echo site_url("billing/schedule/schedule_view");?>');
	...
	$('#provider_list2').change(function() {
		...
		if(id){
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('billing/schedule/set_provider/');?>",
				data: "id=" + id,
				success: function(data){
					$("#provider_schedule1").load('<?php echo site_url("billing/schedule/schedule_view");?>');
				}
