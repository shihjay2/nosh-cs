...
<script type="text/javascript">
	...
	$("#messages_ref_orders").catcomplete({
		...
		select: function(event, ui){
			if (ui.item.value == "*/add/*") {
				$("#messages_ref").val(ui.item.value1);
				$("#messages_ref_orders_text").val(this.value);
				$("#add_test_cpt3").dialog('open');
			} else {
	...
	$("#messages_ref_save").click(function(){
		...
		if (bValid) {
			...
			$.ajax({
				...
				data: "orders_referrals=" + a + "&orders_referrals_icd=" + b + "&address_id=" + c + "&t_messages_id=" + d + "&orders_id=" + e + "&orders_insurance=" + f,
				dataType: "json",
				success: function(data){
					$.jGrowl(data.message);
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('provider/chartmenu/get_ref_templates_list');?>",
		dataType: "json",
		success: function(data){
	...
	$('#messages_ref_template').change(function (){
		...
		if (a != "") {
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('provider/chartmenu/get_ref_template');?>/" + a,
				dataType: "json",
				success: function(data){
