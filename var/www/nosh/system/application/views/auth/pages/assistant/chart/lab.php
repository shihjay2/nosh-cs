...
<script type="text/javascript">
	...
	$("#messages_lab_dialog").dialog({ 
		...
		open: function(){
			 ...
			 jQuery("#messages_lab_list").jqGrid({
				url:"<?php echo site_url('assistant/chartmenu/labs_list');?>",
				postData: {t_messages_id: function(){return $("#messages_lab_t_messages_id").val();}},
				datatype: "json",
				mtype: "POST",
				colNames:['ID','Tests','Diagnosis','Location1','Location','Obtained','Insurance','Provider'],
				colModel:[
			...
			$.ajax({
				...
				success: function(data){
					if(data.response =='true'){
						...
						$("#messages_lab_provider_list").addOption(data.message);
					} else {
						$("#messages_lab_provider_list").addOption({"":"No providers."});
					}
				}
			});
		},
	...
	$("#messages_delete_lab").click(function(){
		...
		if(item){
			$.ajax({
				url: "<?php echo site_url('assistant/chartmenu/delete_lab');?>",
				type: "POST",
				data: "orders_id=" + item,
				success: function(data){
					$.jGrowl(data);
	...
	$("#save_lab_helper").click(function(){
		...
		if (origin == 'message') {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('assistant/chartmenu/import_lab1');?>",
