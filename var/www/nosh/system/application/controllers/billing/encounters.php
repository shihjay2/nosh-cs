...
class Encounters extends Application
{
	...
	function get_previous_versions($eid)
	{
		...
		$html = $this->load->view('auth/pages/billing/chart/encounters/modal_view_insert', $data);
		echo $html;
	}
	...
	function view()
	{
		...
		if ($age <= 6574.5) {
			$data['wcc'] = '<input type="button" id="hpi_wcc" value="Well Child Check" style="font-size: 0.8em" class="nosh_button"><br>';
			$data['cpe'] = '';
			$data['wwe'] = '';
			$data['prenatal'] = '';
			$data['preg'] = '';
			...
			$data['wcc'] = '';
			$data['cpe'] = '<input type="button" id="hpi_cpe" value="Complete Physical" style="font-size: 0.8em" class="nosh_button"><br>';
			if ($gender == 'male') {
				$data['wwe'] = '';
				$data['prenatal'] = '';
				$data['preg'] = '';
			} else {
				$data['wwe'] = '<input type="button" id="hpi_wwe_template" value="Well Woman Exam" class="nosh_button"> ';
				$data['prenatal'] = '<input type="button" id="hpi_prenatal_template" value="Prenatal Visit" class="nosh_button"> ';
				$data['preg'] = '<input type="button" id="hpi_preg" value="Pregnancy Status" style="font-size: 0.8em" class="nosh_button"><br>';
			}
		}
		...
		if ($labsInfo) {
			...
			if ($labsInfo['labs_ua_urobili'] != '' || $labsInfo['labs_ua_bilirubin'] != '' || $labsInfo['labs_ua_ketones'] != '' || $labsInfo->labs_ua_glucose != '' || $labsInfo->labs_ua_protein != '' || $labsInfo->labs_ua_nitrites != '' || $labsInfo->labs_ua_leukocytes != '' || $labsInfo->labs_ua_blood != '' || $labsInfo->labs_ua_ph != '' || $labsInfo->labs_ua_spgr != '' || $labsInfo->labs_ua_color != '' || $labsInfo->labs_ua_clarity != ''){
				...
				if($labsInfo['labs_ua_blood'] != '') {
					$data['labs'] .= '<tr><th align=\"left\">Blood:</th><td align=\"left\">' . $labsInf['labs_ua_blood'] . '</td></tr>';
				}
	...
	{
		$data = $this->modal_view_text($eid);
		$this->load->view('auth/pages/billing/chart/encounters/modal_view_insert', $data);
	}
	...
	function ros()
	{
		...
		$age = $this->session->userdata('agealldays');
		if ($age <= 6574.32) {
			$data['wcc'] = '<tr><td><div id="button_ros_wcc_status">' . $data['wcc_status'] . '</div></td><td><input type="button" id="button_ros_wcc" value="Well Child Visit" class="ui-button ui-state-default ui-corner-all"></td></tr>';
		} else {
			$data['wcc'] = '';
		}	
		if ($gender == 'male') {	
	...
	function oh()
	{
		...
		if ($age > 730.48 && $age <= 6574.32) {
			$data['childcare'] = '<tr><td>Child Care:</td><td><input type="text" name="sh8" id="sh8" style="width:500px" class="text ui-widget-content ui-corner-all"/></td></tr>';
		} else {
			$data['childcare'] = '';
		}
	...
	function vitals()
	{
		...
		if ($agealldays < 6574.5) {
			...
			if ($agealldays > 730.5) {
				$data['graphs'] = '<input type="button" id="weight_chart" value="Weight Growth Chart" class="ui-button ui-state-default ui-corner-all"> <input type="button" id="height_chart" value="Height Growth Chart" class="ui-button ui-state-default ui-corner-all"> <input type="button" id="hc_chart" value="Head Circumference Growth Chart" class="ui-button ui-state-default ui-corner-all"> <input type="button" id="bmi_chart" value="BMI Growth Chart" class="ui-button ui-state-default ui-corner-all"> <input type="button" id="weight_height_chart" value="Weight-Height Growth Chart" class="ui-button ui-state-default ui-corner-all"><br><br>';
			} else {
				$data['graphs'] = '<input type="button" id="weight_chart" value="Weight Growth Chart" class="ui-button ui-state-default ui-corner-all"> <input type="button" id="height_chart" value="Height Growth Chart" class="ui-button ui-state-default ui-corner-all"> <input type="button" id="hc_chart" value="Head Circumference Growth Chart" class="ui-button ui-state-default ui-corner-all"> <input type="button" id="weight_height_chart" value="Weight-Height Growth Chart" class="ui-button ui-state-default ui-corner-all"><br><br>';
			}
		} else {
	...
	function orders_rx_save()
	{
		...
		if ($count > 0) {
			...
			$rx_rx = $row['rx_rx'] . $this->input->post('rx_rx');
			$data = array(
				'eid' => $eid,
				'pid' => $pid,
				'encounter_provider' => $encounter_provider,
	...
	function orders_sup_save()
	{
		...
		if ($count > 0) {
			$row = $this->encounters_model->getRX($eid)->row_array();
			$rx_supplements = $row['rx_supplements'] . $this->input->post('rx_supplements');
			$data = array(
				'eid' => $eid,
				'pid' => $pid,
				'encounter_provider' => $encounter_provider,
			...
			$data = array(
				...
				'pid' => $pid,
				'encounter_provider' => $encounter_provider,
				'rx_supplements' => $this->input->post('rx_supplements'),
			);
			$this->encounters_model->addRX($data);
			$this->audit_model->add();
		...
		if ($item == 'sup'){
			...
			if ($count > 0) {
				...
				if ($data['rx_supplements'] == '') {
					...
					echo nl2br($data['rx_supplements']);
				}
			} else {
				echo 'No entry for this item.';
			}
		...
		if ($item == 'imm'){
			...
			if ($count > 0) {
				...
				if ($data['rx_immunizations'] == '') {
					...
					echo nl2br($data['rx_immunizations']);
				}
			} else {
				echo 'No entry for this item.';
			}
	...
	function billing_save1()
	{
		...
		if ($num_rows5 > 0) {
			...
			$bill_Box28 = $cpt_final[0]['cpt_charge1'] + $cpt_final[1]['cpt_charge1'] + $cpt_final[2]['cpt_charge1'] + $cpt_final[3]['cpt_charge1'] + $cpt_final[4]['cpt_charge1'] + $cpt_final[5]['cpt_charge1'];
			...
			$data1 = array(
				...
				'bill_Box24F1' 				=> $cpt_final[0]['cpt_charge'],	//Charges 1
				'bill_Box24F2'				=> $cpt_final[1]['cpt_charge'],	//Charges 2
				'bill_Box24F3' 				=> $cpt_final[2]['cpt_charge'],	//Charges 3
				'bill_Box24F4' 				=> $cpt_final[3]['cpt_charge'],	//Charges 4
				'bill_Box24F5' 				=> $cpt_final[4]['cpt_charge'],	//Charges 5
				'bill_Box24F6' 				=> $cpt_final[5]['cpt_charge'],	//Charges 6
		...
		echo 'Billing saved and waiting to be submitted!';
	}
	...
	function generate_hcfa()
	{
		...
		if ($query->num_rows() > 0) {
			...
			$user_id = $this->session->userdata('user_id');
			$file_path = "/var/www/nosh/hcfa1500_output_final_" . now() . "_" . $user_id . ".pdf";
...
/* Location: application/controllers/billing/encounters.php */
