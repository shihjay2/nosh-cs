...
class Encounters extends Application
{
	...
	function index()
	{
		...
		if ($eid == FALSE) {
			...
			if ($encounterInfo->num_rows() == 0) {
				...
				redirect('assistant/encounters/view/');
			}
		}
	}
	...
	function new_encounter()
	{
		...
		$this->db->where('displayname', $this->input->post('encounter_provider'));
		$user_query = $this->db->get('users')->row_array();
		$data = array(
			'pid' => $this->session->userdata('pid'),
			'appt_id' => $appt_id,
			'encounter_provider' => $this->input->post('encounter_provider'),
	...
	function get_previous_versions($eid)
	{
		...
		$html = $this->load->view('auth/pages/assistant/chart/encounters/modal_view_insert', $data);
		echo $html;
	}
	...
	function view()
	{
		...
		if ($age <= 365.25) {
			$data['birth'] = '<input type="button" id="hpi_birth_hx_template" value="Birth History" class="ui-button ui-state-default ui-corner-all"> ';
		} else {
			$data['birth'] = '';
		}

		if ($age <= 6574.5) {
			$data['wcc'] = '<input type="button" id="hpi_wcc" value="Well Child Check" style="font-size: 0.8em" class="ui-button ui-state-default ui-corner-all"><br>';
			$data['cpe'] = '';
			$data['wwe'] = '';
			$data['prenatal'] = '';
			$data['preg'] = '';
			...
			$data['wcc'] = '';
			$data['cpe'] = '<input type="button" id="hpi_cpe" value="Complete Physical" style="font-size: 0.8em" class="ui-button ui-state-default ui-corner-all"><br>';
			if ($gender == 'male') {
				$data['wwe'] = '';
				$data['prenatal'] = '';
				$data['preg'] = '';
			} else {
				$data['wwe'] = '<input type="button" id="hpi_wwe_template" value="Well Woman Exam" class="ui-button ui-state-default ui-corner-all"> ';
				$data['prenatal'] = '<input type="button" id="hpi_prenatal_template" value="Prenatal Visit" class="ui-button ui-state-default ui-corner-all"> ';
				$data['preg'] = '<input type="button" id="hpi_preg" value="Pregnancy Status" style="font-size: 0.8em" class="ui-button ui-state-default ui-corner-all"><br>';
			}
		}
		$this->auth->view('assistant/chart/encounters/view', $data);
	}
	...
	{
		$data = $this->modal_view_text($eid);
		$this->load->view('auth/pages/assistant/chart/encounters/modal_view', $data);
	}
	
	function modal_view2($eid)
	{
		$data = $this->modal_view_text($eid);
		$this->load->view('auth/pages/assistant/chart/encounters/modal_view_insert', $data);
	}
	...
	function ros()
	{
		...
		if ($gender == 'male') {	
			$this->load->view('auth/pages/assistant/chart/encounters/ros_male', $data);
		} else {
			$this->load->view('auth/pages/assistant/chart/encounters/ros_female', $data);
		}
	}
	...
	function oh()
	{
		...
		$this->load->view('auth/pages/assistant/chart/encounters/oh', $data);
	}
	...
	function vitals()
	{
		...
		$this->load->view('auth/pages/assistant/chart/encounters/vitals', $data);
	}
	...
	function pe()
	{
		...
		if ($gender == 'male') {	
			$this->load->view('auth/pages/assistant/chart/encounters/pe_male', $data);
		} else {
			$this->load->view('auth/pages/assistant/chart/encounters/pe_female', $data);
		}
	}
	...
	function labs()
	{
		...
		if ($gender == 'male') {	
			$this->load->view('auth/pages/assistant/chart/encounters/labs_male', $data);
		} else {
			$this->load->view('auth/pages/assistant/chart/encounters/labs_female', $data);
		}
	}
	...
	function proc()
	{
		$this->load->view('auth/pages/assistant/chart/encounters/proc');
	}
	...
	function assessment()
	{
		$this->load->view('auth/pages/assistant/chart/encounters/assessment');
	}
	...
	function orders_sup_save()
	{
		...
		$rx_rx = "";
		$this->db->where('eid', $eid);
		$encounter_query = $this->db->get('encounters')->row_array();
		$encounter_provider = $encounter_query['encounter_provider'];
	...
	function orders()
	{
		...
		$this->load->view('auth/pages/assistant/chart/encounters/orders', $data);
	}
	...
	function page_orders1($eid)
	{
		...
		return $this->load->view('auth/pages/assistant/chart/instruction_page',$data, TRUE);
	}
	...
	function billing()
	{
		...
		$this->load->view('auth/pages/assistant/chart/encounters/billing', $data);
	}
...
/* Location: application/controllers/assistant/encounters.php */
