...
class Chartmenu extends Application
{
	...
	function menu()
	{
		...
		$data['id'] = $pid;
		$this->load->view('auth/pages/assistant/chart/menu', $data);
	}
	...
	{
		$this->load->view('auth/pages/assistant/chart/issues');
	}
	
	function rx_load()
	{
		$this->load->view('auth/pages/assistant/chart/rx');
	}
	
	function supplements_load()
	{
		$this->load->view('auth/pages/assistant/chart/supplements');
	}
	
	function imm_load()
	{
		$this->load->view('auth/pages/assistant/chart/imm');
	}
	
	function lab_load()
	{
		$this->load->view('auth/pages/assistant/chart/lab');
	}
	
	function rad_load()
	{
		$this->load->view('auth/pages/assistant/chart/rad');
	}
	
	function cp_load()
	{
		$this->load->view('auth/pages/assistant/chart/cp');
	}
	
	function ref_load()
	{
		$this->load->view('auth/pages/assistant/chart/ref');
	}
	
	function billing_load()
	{
		$this->load->view('auth/pages/assistant/chart/billing');
	}
	
	function print_load()
	{
		$this->load->view('auth/pages/assistant/chart/print');
	}
	
	function letter_load()
	{
		$this->load->view('auth/pages/assistant/chart/letter');
	}
	
	function mtm_load()
	{
		$this->load->view('auth/pages/assistant/chart/mtm');
	}
		...
		if($this->input->post('rxl_id') == '') {
			...
			if ($add) {
				...
				$result = array(
					...
					'medtext' => $this->input->post('rxl_medication') . ' ' . $this->input->post('rxl_dosage') . ' ' . $this->input->post('rxl_dosage_unit') . ', ' . $instructions . ' for ' . $this->input->post('rxl_reason')
				);
			} else {
	...
	function page_medication($rxl_id)
	{
		...
		return $this->load->view('auth/pages/assistant/chart/prescription_page',$data, TRUE);
	}
	...
	function print_medication($rxl_id)
	{
		...
		$this->mpdf->rximage = file_get_contents('/var/www/nosh/images/rxicon.png');
		$this->mpdf->Image('var: rximage',30,30);
		//practice logo here
	...
	function send_fax_medication()
	{
		...
		$job_id = $this->session->userdata('job_id');
		$meta = array("(", ")", "-", " ");
		$fax = str_replace($meta, "", $this->input->post('messages_pharmacy_fax_number'));
		if ($fax != '') {
			$send_list_data = array(
				'faxrecipient' => $this->input->post('messages_pharmacy_name'),
				'faxnumber' => $fax,
				'job_id' => $job_id
			);
			$this->fax_model->addSendList($send_list_data);
	...
	function page_medication_list()
	{
		...
		$data['letter'] = $body;
		return $this->load->view('auth/pages/assistant/chart/letter_page',$data, TRUE);
	}
	...
	function page_letter($letter_to, $letter_body, $address_id, $user_id)
	{
		$body = '';
		if ($address_id != '') {
			$this->db->where('address_id', $address_id);
		...
		$this->db->select('displayname');
		$this->db->where('id', $user_id);
		$displayname_row = $this->db->get('users')->row_array();
		$this->db->select('signature');
		$this->db->where('id', $user_id);
		$signature_query = $this->db->get('providers');
		if ($signature_query->num_rows() > 0) {
			$signature = $this->db->get('providers')->row_array();
			$signature1 = str_replace("/var/www/nosh/","", $signature['signature']);
			$sig = "<img src='" . base_url() . $signature1 . "' border='0'><br>" . $displayname_row['displayname'];
		} else {
			$sig = '<br><br><br><br><br><br><br>' . $displayname_row['displayname'];
		}
	...
	function print_letter()
	{
		...
		$address_id = $this->input->post('address_id');
		$provider_id = $this->input->post('letter_provider');
		$footer = '<div style="border-top: 1px solid #000000; text-align: center; padding-top: 3mm; font-size: 8px;">Page {PAGENO} of {nb}</div>';
		$footer .= '<p style="text-align:center; font-size: 8px;">';
		$footer .= "CONFIDENTIALITY NOTICE: The information contained in this facsimile transmission is intended for the recipient named above. If you are not the intended recipient or the intended recipient's agent, you are hereby notified that dissemination, copying, or distribution of the information contained in the transmission is prohibited. If you are not the intended recipient, please notify us immediately by telephone and return the documents to us by mail. Thank you.";
		$footer .= '</p><p style="text-align:center; font-size: 8px;">This document was generated by NOSH ChartingSystem</p>';
		$html = $this->page_intro('Letter');
		$html .= $this->page_letter($letter_to, $letter_body, $address_id, $provider_id);
		$this->load->library('mpdf');
		$this->mpdf=new mpdf('','Letter',0,'',26,26,26,26,9,9,'P');
		$this->mpdf->useOnlyCoreFonts = true;
		$this->mpdf->shrink_tables_to_fit=1;
	...
	{
		$result = $this->billing_save_common($insurance_id_1, $insurance_id_2);
		$pid = $this->session->userdata('pid');
		$this->db->where('eid', $eid);
	...
	function encounters_view($eid)
	{
		...
		return $this->load->view('auth/pages/assistant/chart/encounters_view', $data, TRUE);
	}
...
/* Location: application/controllers/assistant/chartmenu.php */
